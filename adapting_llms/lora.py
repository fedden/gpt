...
class LoraLinear(torch.nn.Module):
    """A linear layer that implements a Low Rank Approximation (LoRA).
    ...
    def __init__(
        ...
        self.A = torch.nn.Linear(
            self.original_layer.in_features, self.rank, bias=False,
        )
        self.B = torch.nn.Linear(
            self.rank, self.original_layer.out_features, bias=False,
        )
        self._initialize_weights()

    def _initialize_weights(self) -> None:
        """Initialize the weights of the linear layer."""
        # Note: the original paper mentions that normal distribution was used
        # for initialization. However, the official LoRA implementation uses
        # "Kaiming/He Initialization".
        self.A.weight.data.normal_(mean=0.0, std=math.sqrt(2.0 / self.A.in_features))
        # Initialize B with zeros so that the output of the layer is initially
        # zero - see section 4.1 of the paper.
        self.B.weight.data.zero_()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass of the linear layer."""
        x_lora: torch.Tensor = self.B(self.A(x)) * (self.alpha / self.rank)
        x_original: torch.Tensor = self.original_layer(x)
        return x_lora + x_original
...
class CausalSelfAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        # Convert to LoRA.
        self.query_projection_lora = LoraLinear(
            original_layer=self.query_projection,
            rank=4,
            alpha=32.0,
        )
        self.value_projection_lora = LoraLinear(
            original_layer=self.value_projection,
            rank=4,
            alpha=32.0,
        )
        # Regularization
...
class GptTransfomer(pl.LightningModule):
    ...
    def __init__(
        ...
        # Freeze all parameters except any associated with `LoraLinear`.
        for name, parameters in self.named_parameters():
            if not fnmatch.fnmatch(name, "*.*_lora.A.*") or fnmatch.fnmatch(
                name, "*.*_lora.B.*"
            ):
