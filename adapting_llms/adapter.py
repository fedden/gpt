...
class Adapter(torch.nn.Module):
    """Adapter module from https://arxiv.org/pdf/1902.00751.pdf."""

    def __init__(self, n_embedding_dims: int, n_bottle_neck_dims: int):
        super().__init__()
        # The adapter is a simple feed-forward network with a bottleneck layer
        # in the middle. The bottleneck layer is used to reduce the number of
        # parameters in the network.
        self.adapter = torch.nn.Sequential(
            torch.nn.Linear(n_embedding_dims, n_bottle_neck_dims),
            torch.nn.ReLU(),
            torch.nn.Linear(n_bottle_neck_dims, n_embedding_dims),
        )
...
class GptTransformerBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.adapter_0 = Adapter(
            n_embedding_dims=n_embedding_dims,
            n_bottle_neck_dims=24,
        )
        self.adapter_1 = Adapter(
            n_embedding_dims=n_embedding_dims,
            n_bottle_neck_dims=24,
        )
...
class GptTransfomer(pl.LightningModule):
    ...
    def __init__(
        ...
        # Freeze all parameters except any associated with the class `Adapter`.
        for name, parameters in self.named_parameters():
            if not fnmatch.fnmatch(name, "*.*adapter*"):
